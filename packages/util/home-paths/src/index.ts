/**
 * Shared filesystem path helpers for Mintal Agent user data.
 *
...
/** Directory name for the default Mintal Agent home under the OS home. */
export const DSH_HOME_DIR_NAME = '.mintal-agent'

/** Stable user-facing display form for the default Mintal Agent home. */
export const DEFAULT_DSH_HOME_DISPLAY = `~/${DSH_HOME_DIR_NAME}`

/** Environment variable that overrides the default Mintal Agent home. */
...
/**
 * Resolve the default Mintal Agent home using Node's platform path rules.
...
/**
 * Resolve the single-root Mintal Agent home.
 *
 * Precedence, highest first: an explicit configured path, `$DSH_HOME`, then
 * `~/.mintal-agent`. The application keeps all user data under one root. An empty or
...
/**
 * Join path segments onto the resolved Mintal Agent home.
 * @param segments - path segments appended to the Mintal Agent home; an empty list returns the home itself.
 ...
 * `~/.mintal-agent`, and any configured home is labelled `$DSH_HOME`.
 * @param resolvedHome - the absolute path returned by {@link resolveDshHome}.
 * @returns `~/.mintal-agent` for the default home, otherwise `$DSH_HOME`.
 */
